...
export class SubscriptionsListComponent implements OnInit {
  ...
  private _getDataForPdfTable() {
    const data = [];
    this.exportedStudents = this.subscriptions.filter(function(sub) {
      return sub.select == true;
    });
    for (let i = 0; i < this.exportedStudents.length; i++) {
      data.push({
        First_Name: this.exportedStudents[i].studentFirstName,
        Last_Name: this.exportedStudents[i].studentLastName,
        Assessment: this.exportedStudents[i].assessment,
        Date_Registered: this.exportedStudents[i].dateOfRegistration
      });
    }
    return data;
  }
  ...
  public openPDF(): void {
    ...
    doc.text('Registered students', 30, 20);
    ...
    doc.setFontSize(12);
    doc.table(10, 35, this._getDataForPdfTable(), SubscriptionsListComponent._createHeadersForPdfTable([
      'First_Name', 'Last_Name', 'Assessment', 'Date_Registered'
    ]), { autoSize: false });
    doc.save(fileName);
  }
