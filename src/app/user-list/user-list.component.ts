...
export class UserListComponent implements OnInit {
  ...
  public userWordFormatter(): string{
  ...
  private _getDataForPdfTable() {
    const data = [];
    this.exportedUsers = this.users.filter(function(user) {
      return user.select == true;
    });
    for (let i = 0; i < this.exportedUsers.length; i++) {
      data.push({
        First_Name: this.exportedUsers[i].firstName,
        Last_Name: this.exportedUsers[i].lastName,
        Phone: this.exportedUsers[i].phone,
        Email: this.exportedUsers[i].email
      });
    }
    return data;
  }
  ...
  public openPDF(): void {
    ...
    doc.setFontSize(12);
    doc.table(10, 35, this._getDataForPdfTable(), UserListComponent._createHeadersForPdfTable([
  ...
  public deleteUsers(){
    this.deletedList = this.users.filter(function(user) {
      return user.select == true;
    });
    for(const u of this.deletedList){
      this.removeUser.emit(u.id);
    }
  }
  ...
  onChange($event){
    ...
    if(this.users.every(user => user.select == false || user.select == undefined)){
